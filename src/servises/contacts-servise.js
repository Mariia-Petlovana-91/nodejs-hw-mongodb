...
export const updateContact = async (_id, payload) => {
	const result = await ContactCollections.findOneAndUpdate( { _id}, payload,{new:true});
	return result;
};
export const deleteContact = filter => ContactCollections.findOneAndDelete(filter);
